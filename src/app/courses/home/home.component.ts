...
export class HomeComponent implements OnInit {
  ...
  constructor(
    ...
    private courseEntityService: CourseEntityService // will be used to get the data from BE but also query it in the store
  ) {}
  ...
  reload() {
    ...
    this.promoTotal$ = this.courseEntityService.entities$.pipe(
      map((entities) => entities.filter((course) => course.promo).length)
    );
  }
